...
module.exports.controller = (app, io, socket_list ) => {
    ...
    app.post('/api/sign_up', (req, res) => {
        ...
        helper.CheckParameterValid(res, reqObj, ["name", "mobile", "email", "email", "address", "password","push_token", "device_type"], () => {
            ...
            db.query('SELECT `user_id`, `email`, `created_date`, `update_date`, `status` FROM `user_detail` WHERE `email` = ? AND  `password` = ? AND `status` = "1" ', [ reqObj.email, reqObj.password ] , (err, result) => {
                ...
                if(result.length > 0) {
                    ...
                    db.query('INSERT INTO `user_detail` (`email`, `password`, `created_date`, `update_date`, `status`) VALUES (?, ?, ?, ?, ?)', [ reqObj.email, reqObj.password, helper.DateTime(), helper.DateTime(), "1" ] , (err, result) => {
